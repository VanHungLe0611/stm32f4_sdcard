...
# linker flags
linkflags = [
    '-mcpu=cortex-m4'     , '-mthumb'         , '-TSTM32F407VGTx_FLASH.ld' , '-fsigned-char'        ,
...
cppdefines = [ 'USE_HAL_DRIVER', 'STM32F407xx', 'HSE_VALUE=8000000',  'USE_FULL_LL_DRIVER']
...
for folder in list(set(src_folders)): # avoid duplicates
    ...
    if not os.path.exists(sconscript_name):
        ...
        objs.append(o)
objs.append(env.Object('startup_stm32F407xx.s'))
###### end add src folders ######
